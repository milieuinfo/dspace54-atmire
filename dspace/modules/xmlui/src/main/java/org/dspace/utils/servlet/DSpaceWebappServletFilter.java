...
public final class DSpaceWebappServletFilter implements Filter {
	...
	private static final Logger log = Logger
			.getLogger(DSpaceWebappServletFilter.class);

	private List<String> pathsToBeIgnored;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		if (pathShouldBeIgnored((HttpServletRequest)request)) {
			// Continue processing
			if (log.isDebugEnabled()) {
				String path = ((HttpServletRequest) request).getRequestURI();
				log.debug("The following path is bypassing the dspace filter: "
						+ path);
			}
	...
	private boolean pathShouldBeIgnored(HttpServletRequest request) {
		String path = request.getRequestURI().substring(request.getContextPath().length());
		for (String pattern : pathsToBeIgnored) {
